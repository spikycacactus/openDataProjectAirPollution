...
def draw(which_AQI, minlat, minlng, maxlat, maxlng):
    requested_AQI_position = TYPES[which_AQI]["cislo"]
    # vymaze staru mapku
    plt.clf()
    # vykresli mapku
    ...
    for typ in climate:
        ...
        if percentage == 1:
            ...
            cnv.create_text(10, y, fill=climate[typ]['color'], text = 'Only Good' +': ' + str(current_number_of_stations), anchor = 'w', font=NORMAL)
            break
        else:
            cnv.create_arc(25, 25, 150, 150, start=start_of_arc, extent=percentage * 360, fill=climate[typ]['color'], outline="")
        start_of_arc += percentage * 360
        if typ == 'Unhealthy for Sensitive Groups':
            cnv.create_text(10, y, fill=climate[typ]['color'], text = 'Unhealthy for Senstive' +': ' + str(current_number_of_stations), anchor = 'w', font=NORMAL)
        else:
            cnv.create_text(10, y, fill=climate[typ]['color'], text= typ + ': ' + str(current_number_of_stations), anchor='w', font=NORMAL)
    ...
    for i in range(len(least_pollutted_cities)):
        y+=15
        cnv.create_text(200, y, fill='black', text=least_pollutted_cities[i][1] + ': ' + str(least_pollutted_cities[i][0]), anchor='w', font= NORMAL)
        cnv.create_text(350, y, fill='black', text=most_pollutted_cities[i][1] + ': ' + str(most_pollutted_cities[i][0]), anchor='w', font = NORMAL)
    ...
    cnv.create_text(15, y + 100, text = TYPES[which_AQI]["popisok"], anchor='w', font= BOLD)
    # kresli mapu
    plt.show()
...
currentregion = "World"
def draw_action():
    #zisti selktnuty index a povie aky region je oznaceny, nasledne spusti funkciu an vykreslovanie
...
def choose(event):
    ...
    for line in data:
        ...
        if current[1] == chosen:
            ...
            for typ in TYPES:
                y += 15
                cnv2.create_text(10, y, fill=CLIMATE_COLORS[current[TYPES[typ]["cislo"]]], text=typ + ': ' + str(current[TYPES[typ]["cislo"]]), anchor='w', font=NORMAL)
                cnv2.create_text(255, y, fill=CLIMATE_COLORS[current[TYPES[typ]["cislo"]]], text='value: ' + str(current[TYPES[typ]["cislo"] + 1]), anchor='w', font=NORMAL)
            break
